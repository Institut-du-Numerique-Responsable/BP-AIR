---
id: I2
titre: Politique d'achats numériques responsables
theme: Infrastructure & Matériel
statut: brouillon
owner: ""
contributeurs: []
reviewers: []
version: 0.1
maj: 2026-06-04
fiches_liees: [G4, I1, V1]
---

# I2 — Politique d'achats numériques responsables

> **Public cible.** Achats, DSI, architectes, juristes.

## Objectif

Encadrer l'acquisition de produits et services numériques pour réduire l'empreinte tout en garantissant performance économique et technique.

## Contexte & enjeu

**80 % de l'empreinte d'un équipement est figée à la fabrication** : acheter du neuf « performant » sans avoir évalué le réemploi est un contresens. Sans les Achats, aucune politique matérielle ne tient. Cf. [guide unifié](../guide-unifie.md#25-pilier-4-cycle-de-vie-du-materiel-economie-circulaire).

## Étapes de mise en œuvre

1. Poser les **3 piliers** : réduction à la source · critères Green IT (réparabilité, TCO/EPEAT) · responsabilité sociale (droits humains, ESAT).
2. Intégrer le **Clausier NR de l'INR** dans les appels d'offres (sélection : questionnaires ; exécution : clauses CCTP/CCAP), segmenté par type d'achat (matériel, logiciel/SaaS, hébergement/cloud, prestations).
3. Exiger la conformité au **RGESN** pour tout nouveau service.
4. S'appuyer sur le **dispositif RFAR** (charte + label d'État adossé à l'ISO 20400).

## KPIs & OKR

- **KPI** : % d'appels d'offres intégrant des clauses NR ; % de matériel reconditionné dans les nouvelles acquisitions.
- ***OKR*** : politique d'achats validée par Achats + DSI ; +50 % de matériel reconditionné.

## Pièges à éviter

- Des clauses NR « décoratives » non évaluées ni contractualisées.
- Acheter du neuf « performant » sans avoir évalué le réemploi (fabrication = 80 % de l'empreinte).
- Négliger la dimension sociale au profit du seul carbone.

## Outils & ressources

| Outil / Ressource | Usage | Lien |
|---|---|---|
| Clausier NR (INR) | Clauses CCTP/CCAP | <https://institutnr.org/clausier-numerique-ecoresponsable> |
| Dispositif RFAR | Charte + Label d'État (ISO 20400) | <https://www.economie.gouv.fr/mediateur-des-entreprises> |
| Guide achats responsables (DINUM) | Achats | <https://ecoresponsable.numerique.gouv.fr/publications/guide-pratique-achats-numeriques-responsables/> |

## Fiches liées

- [G4 — Construire la feuille de route](G4-feuille-de-route.md)
- [I1 — Optimiser infrastructures & environnements](I1-infrastructures.md)
- [V1 — Évaluer la maturité des parties prenantes](V1-maturite-parties-prenantes.md)

---

!!! note "🗨️ Notes de coédition (à purger avant validation)"
    —
